...
}:
{
  opts.funcs.containers = {
    ...
    mkUidMaps = n: [
      "0:${toString (1 + vars.containers.uidGidCount * n)}:${toString vars.containers.uidGidCount}"
    ];
    ...
    mkGidMaps =
      ...
      ++ (map (
        g:
        "${funcs.containers.getContainerGid g}:${
          toString (
            if config.users.groups.${g}.name == "users" then
              "0"
            else
              (config.users.groups.${g}.gid - vars.containers.startGid + 1)
          )
        }:1"
      ) (builtins.filter (x: x != null) gs));

    mkAddGroups = map (g: "${funcs.containers.getContainerGid g}");
    mkUser =
      user: group:
      if user != null then
        "${user}${if group != null then ":${funcs.containers.getContainerGid group}" else ""}"
      else
        null;

    mkConfig =
      user: containerVars: extraConfig:
      lib.recursiveUpdate {
        autoStart = false;
        serviceConfig = {
          RestartSec = "10";
        ...
        containerConfig = {
          ...
          uidMaps = funcs.containers.mkUidMaps containerVars.i;
          gidMaps = funcs.containers.mkGidMaps containerVars.i (
            [ containerVars.mainGroup ] ++ containerVars.extraGroups
          );
          addGroups = funcs.containers.mkAddGroups containerVars.extraGroups;
        };
      } extraConfig;
  };
}
